...
#                     glass_blur, motion_blur, zoom_blur, fog,
#                     brightness, contrast, elastic_transform, pixelate,
#                     jpeg_compression, speckle_noise, gaussian_blur, spatter,
#                     saturate,
#                     darkness, lens_distortion, resolution_change,
#                     specular_reflection, color_changes)
...
def get_corruption_names(subset='all'):
    ...
    elif subset == 'lighting':
        return [f.__name__ for f in corruption_tuple[:4]]
   
    elif subset == 'movement':
        return [f.__name__ for f in corruption_tuple[4:8]]
    
    elif subset == 'preprocessing':
        return [f.__name__ for f in corruption_tuple[8:12]]

    elif subset == 'endoscopy':
        return [f.__name__ for f in corruption_tuple[12:]]
    
    else:
        raise ValueError("subset must be one of ['lighting', 'movement', 'preprocessing', 'preprocessing', 'all']")
